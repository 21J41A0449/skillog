...
import { GlobeIcon, LockIcon, GitHubIcon, ExternalLinkIcon, CloseIcon } from '../components/icons';
...
const DashboardLogCard = ({ log, currentUser, onDelete }: { log: LogEntry, currentUser: User, onDelete?: (logId: string) => void }) => {
        ...
        <div className="bg-surface p-4 rounded-xl border border-border">
            <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {isOwnLog && onDelete && (
                        <button onClick={handleDeleteClick} className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-red-400 hover:text-red-500 transition-colors">
            ...
            <div>
                ...
                {userLogs.length > 0 ? (
                    <div className="space-y-4">
                        {userLogs.map(log => (
                            <DashboardLogCard key={log.id} log={log} currentUser={currentUser} onDelete={onDeleteLog} />
                        ))}
                    </div>
                ) : (
